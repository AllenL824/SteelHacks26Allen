...
A 'repetition' flag (or the transcript showing the same word twice in a row, or a cut-off fragment like 's-' or 'th-' before a word) means the reader repeated something. You MUST output an event for EVERY 'repetition' flag — classify it as word_repetition when a whole word recurs, or sound_repetition when only a sound/syllable or fragment recurs. Never drop a repetition flag and never mark it false_alarm. Prefer repetition over prolongation whenever the same word or sound recurs.
